...
class UserProfileModel extends Model
{
    ...
    {
        try{
            $friendsList = $this->UIDtable->getChild('/friendList')->getChildKeys();
            $userList = array();
    ...
    public function userDetails(array $var = null)
    {
        ...
        try{
            ...
            $arr = $this->refineObject($this->UIDtable->getSnapshot()->getValue());
            return response()->json([
                'status' => 'success',
    ...
    public function profileImages(array $var = null)
    {
        ...
        try{
            ...
            if(empty($profileImages)){
                ...
                $this->UIDtable->getChild('/profileImages')->getChild($var['image_id'])->set($var);
                // return $this->UIDtable->getChild('/profileImages')->getChild($var['filename'])->getSnapshot()->getValue();
                return response()->json([
                    'status' => 'success',
                    'message' => 'Image Successfully inserted',
                    'data' => $this->UIDtable->getChild('/profileImages')->getChild($var['image_id'])->getSnapshot()->getValue(),
    ...
    public function userPreferenceAgeRange(array $var = null)
    {
        try {
            ...
            $data = $this->UIDtable->getChild('preference')->getChild('ageRange')->set(['start_age'  => $var['start_age'],
            'end_age'  => $var['end_age']]);
            if($ageRange->wasRecentlyCreated){
                return response()->json([
                    'status' => 'success',
                    'message' => 'Age Range Successfully added',
                    'data' => $data->getSnapshot()->getValue(),
                ]);
            }else{
                return response()->json([
                    'status' => 'success',
