...
//end of Send Notification
    ...
    function check_if_exist_post_with_user(array $cpost=null,$uid=null)
    {
      //  $requestdata = $this->requestList->getChild($post_id)->getValue();
        foreach($cpost as $key=>$value)
        {
            
            if($key==$uid && $value['status']=='accepted')
            {
            return true;
            }
    ...
    {
        $requestdata = $this->requestList->getSnapshot()->getValue();
        foreach($requestdata as $key=>$value)
        {
          
           $match=$this->check_if_exist_post_with_user($requestdata[$key],$var['request_by_uid']);
           if($match)
           {
            $request =  $this->requestList->getChild($key);
            $request->getChild($var['request_by_uid'])->remove(); 
           }
        }
        try {
            $created_at = array(
            ...
            ///delete the other accepted request if any other is going to be accept
...
//end of cancel present request
    ...
    public function show($id)
    {
       ...
       $reqdata= $this->requestList->getChild($id)->getChild($this->firebase_uid)->getSnapshot()->getValue();
